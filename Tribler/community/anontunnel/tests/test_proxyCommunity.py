...
class TestProxyCommunity(TestAsServer):
    ...
    def setUp(self):
        ...
        def _setUp():
            ...
            self.community = dispersy.define_auto_load(ProxyCommunity, dispersy_member, (None, settings, self.session.lm.rawserver), load=True)[0]
    ...
    def test_create_circuit(self):
        ...
        self.assertEqual(create_sender.sock_addr, circuit.first_hop)
        ...
        self.assertEqual(create_sender.sock_addr, destination)
    ...
    def test_on_created(self):
        ...
        self.community.on_created(circuit.circuit_id, first_hop.sock_addr, CreatedMessage([]))
    ...
    def test_on_extended(self):
        ...
        result = self.community.on_created(circuit.circuit_id, first_hop.sock_addr, CreatedMessage([]))
        ...
        result = self.community.on_created(circuit.circuit_id, first_hop.sock_addr, CreatedMessage(candidate_list))
        ...
        self.assertEqual(first_hop.sock_addr, circuit_candidate)
        ...
        result = self.community.on_extended(circuit.circuit_id, first_hop.sock_addr, ExtendedMessage("", []))
        self.assertTrue(result)
        self.assertEqual(CIRCUIT_STATE_READY, circuit.state)
    ...
    def test_on_data(self):
        ...
        self.community.on_data(circuit.circuit_id, first_hop.sock_addr, data_message)
        ...
        self.community.on_data(1337, first_hop.sock_addr, exit_message)
        on_exiting_from_tunnel.assert_called_with(1337, first_hop.sock_addr, destination, payload)
    ...
    def test_on_extend(self):
        ...
        self.community.on_extend(originator_circuit_id, originator.sock_addr, ExtendMessage(extend_pub_key))

        # Check whether we are sending a CREATE to node_to_extend_with
        args, kwargs = send_message.call_args
        create_destination, circuit_id, message_type, message = args
        self.assertEqual(node_to_extend_with.sock_addr, create_destination)
        self.assertEqual(MESSAGE_CREATE, message_type)
    ...
    def test_on_pong(self):
        ...
        result = self.community.on_pong(circuit.circuit_id, first_hop.sock_addr, PongMessage())
        self.assertTrue(result)
    ...
    def test_on_ping(self):
        ...
        self.community.on_create(circuit_id, first_hop.sock_addr, CreateMessage())

        self.community.send_message = send_message = Mock()
        self.community.on_ping(circuit_id, first_hop.sock_addr, PingMessage())

        # Check whether we responded with a pong
        args, kwargs = send_message.call_args

        self.assertEqual(first_hop.sock_addr, kwargs['destination'])
